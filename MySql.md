...
总结：连接两张表中的数据，仅取满足on条件的数据

### 3.1.2 外连接 left join，left outer join ，right join， right outer join， union

#### 3.1.2.1 left join and left outer join

取笛卡尔积中满足on条件的数据加上左表剩余的数据

#### 3.1.2.2 right join and right outer join

取笛卡尔积中满足on条件的数据加上右表剩余的数据

#### 3.1.2.3 full outer join

取笛卡尔积中满足on条件的数据加上左表剩余的数据和右表剩余的数据，即两张表一次左连接一次右连接，然后将两次连接的结果集合并

注意：mysql中不支持full outer join（full join），可以用union替代
